...
class Zhugua :
    ...
    def makeGuaByYaostring(self, yaostring):   
        #yaostring=" 123012", 0为老阴，1少阳 2少阴 3 老阳
        #注意，这个方法第1个数字是六爻
    ...
    def displayDoubleGuaText(self,has_yaochi=True):  #前置条件必须设置了摇卦日期 setDate, 否则无法计算六神
        ...
        if self.isJingGua==False:
           # print("　"*10+self.guaname1 +self.gui1+ "　"*10+self.guaname2+self.gui2)
            ...
            textbuffer.append(f"         {self.guaname1}静卦  {self.gui1}")
...
def statistics_2_active():
    import csv
    fp=open("brother_son.csv","w",newline="")
    writer=csv.writer(fp)
        ...
        for num in gonglist:
            for i in range(0,6):
                for j in range(i+1,6):
                    ...
                    if (six1=="兄弟" and six12=="兄弟") and (six2=="子孙" and six22=="父母"):
...
def paipan(day_str,gua_name_str):
    ...
    outGuaName,guacont=gua.displayDoubleGuaText(has_yaochi=True)
    return guacont
    
def countGongByName(gua_name_str):
    #根据卦名计算卦宫，返回卦宫名字
    if '之' in gua_name_str:
        split_string=gua_name_str.split('之')
        name1 = split_string[0].strip() 
...
if __name__ == "__main__": 
    # day1="2025-08-27"
    # guacont=paipan(day1,"泰静卦")
    # print(guacont)
    #print(countGongByName("豫静卦"))
    print("Let us start working...")
    statistics_2_active()
