...
def changeName(path):
    ...
    for filename in os.listdir(path):
        i=i+1
        if "KO_" in filename:
            newfilename=filename.replace("KO_","")
        #print(newfilename)
...
def changeName2(path):
    os.chdir(path)
    i=0
    for filename in os.listdir(path):
        i=i+1
        if "汇总" in filename:
            newfilename="Total_"+filename
        #print(newfilename)
            os.rename(filename,newfilename)
            print(i,filename, newfilename)
    print("OK! finished! ")

CURRENT_DIR="D:/Markdown/2019己亥年/"
...
if __name__ == "__main__":
   ...
   # dir=CURRENT_DIR
    ...
    '''
            #escaped_text = html.escape(self.content_md_format)
            #html_cont = escaped_text.replace("\n", "<br>")
            #html_cont = re.sub(r'<br>s*<br>s*<br>', '<br>', html_cont, flags=re.IGNORECASE)
